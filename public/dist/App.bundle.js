...
function searchResultsHtml(stores) {
	return stores.map(function (store) {
		return '<a href=\'/store/' + store.slug + '\' class=\'search__result\'>' + store.name + '</a>';
	}).join('');
}
...
function typeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		_axios2.default.get('/api/search?q=' + this.value).then(function (res) {
			...
			searchResult.innerHTML = _dompurify2.default.sanitize('<div class=\'search__result\'>No Results found for ' + _this.value + '</div>');
		}).catch(function (err) {
			console.Error(err);
		});
	});
...
(0, _autocomplete2.default)((0, _bling.$)('#address'), (0, _bling.$)('#lat'), (0, _bling.$)('#lang'));
